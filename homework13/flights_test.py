...
class FlightsTests(unittest.TestCase):
    ...
    def test_00_doctest(self):
        failures, tests = doctest.testmod(flights, verbose=False)
        self.assertEqual(failures, 0)
        self.assertEqual(tests, 4)
        FlightsTests.Points += 0.5

    def test_01_mypy(self):
        _, _, exit_status = mypy.api.run(['flights.py'])
        self.assertTrue(exit_status == 0)
        FlightsTests.Points += 0.5

    def test_02_read_graph(self):
        cases = (
    ...
    def test_03_find_cheapest_flight_plan(self):
        cases = (
