...
class SimCityTests(unittest.TestCase):
    ...
    def test_00_doctest(self):
        failures, tests = doctest.testmod(sim_city, verbose=False)
        self.assertEqual(failures, 0)
        self.assertEqual(tests, 4)
        SimCityTests.Points += 0.5

    def test_01_mypy(self):
        _, _, exit_status = mypy.api.run(['sim_city.py'])
        self.assertTrue(exit_status == 0)
        SimCityTests.Points += 0.5

    def test_02_read_points(self):
    ...
    def test_04_construct_mst(self):
        cases = (
            (self.Input1, {1: 0, 2: 1}),
