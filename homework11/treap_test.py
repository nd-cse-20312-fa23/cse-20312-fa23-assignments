...
class TreapTests(unittest.TestCase):
    ...
    Tree1  = Node('B', 2, 0, Node('A', 1), Node('C', 3))
    Tree2  = Node('A', 1, 0, None, Node('B', 2, 0, None, Node('C', 3)))
    Tree3  = Node('C', 3, 0, Node('B', 2, 0, Node('A', 1)))

    Tree4  = Node('D', 4, 0, Node('B', 2, 0, Node('A', 1), Node('C', 3)), Node('E', 5))
    Tree5  = Node('B', 2, 0, Node('A', 1), Node('D', 4, 0, Node('C', 3), Node('E', 5)))

    Tree6  = Node('F', 1, 76, Node('C', 3, 70, Node('A', 4, 55), Node('D', 0, 60)), Node('H', 2, 14))
    Input6 = (('D', 60), ('F', 76), ('H', 14), ('C', 70), ('A', 55))

    Tree7  = Node('E', 1, 50, Node('D', 3, 30), Node('G', 2, 25, Node('F', 0, 10)))
    ...
    def test_00_doctest(self):
        failures, tests = doctest.testmod(treap, verbose=False)
        self.assertEqual(failures, 0)
        self.assertEqual(tests, 7)
        TreapTests.Points += 0.5

    def test_01_mypy(self):
        _, _, exit_status = mypy.api.run(['treap.py'])
        self.assertTrue(exit_status == 0)
        TreapTests.Points += 0.5

    def test_02_rotate_left(self):
